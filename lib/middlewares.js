...
/** @param {CtxT} ctx */
export const noPM = ({ chat: { type }, reply }, next) => {
...
/** @param {CtxT} ctx */
export const report = async ({ message, telegram: tg, chat, from, reply }) => {
  const admins = await tg.getChatAdministrators(chat.id)
  const reportedMsg = message.reply_to_message
  const dbGroup = await db.getGroup(chat.id)
  ...
  if (dbGroup && dbGroup.legal) {
    api.addUser({
      ...
      last_name,
      username,
      reason: `Spam, repobted by ${from.id}` +
...
/** @param {CtxT} ctx */
export const newMember = async ({ message, telegram: tg, reply, chat }) => {
  const me = await tg.getMe()
  const newMembers = message.new_chat_members

  // if someone added ME
  // send chat info to my PM
  if (newMembers.findIndex(({ id }) => id === me.id) !== -1) {
    await reply(REPLICAS.group_welcome)
    const addedInDB = await db.addGroup({
      id: chat.id,
    ...
    addedInDB && (await tg.sendMessage(ADMIN_UID, newGroupMsg, {
      parse_mode: 'HTML'
    }))
  }
