...
export const onNewMember: OnMiddleware<"new_chat_members"> = async function (
  ctx,
  next,
) {
  ...
  if (dbGroup && dbGroup.voteban_to_global) {
    const banResults = await Promise.all(
      newMembers.map(
        async (member) =>
          [member, Boolean(await api.checkUser(member.id))] as [User, boolean],
      ),
    );
...
export const adminPermissionCBQuery: Middleware = async function (
  ctx,
  next: any,
) {
  const { from } = ctx;
 ...
 */
export const checkIfBotIsAdmin =
  (verbose: boolean): Middleware =>
  async (ctx, next) => {
    let isAdmin = false;
    try {
