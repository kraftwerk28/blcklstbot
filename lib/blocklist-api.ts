...
export class Api {
  ...
  async addUser(banInfo: BanInfo): Promise<boolean> {
    return this.axios.post('/blocklist', banInfo).then(() => true, () => false);
  }

  async checkUser(userID: number): Promise<BannedUser | null> {
    return this.axios.get(`/blocklist/${userID}`).then(r => r.data, () => null);
  }

  async rmUser(userID: number): Promise<boolean> {
    return this.axios.delete(`/blocklist/${userID}`)
      .then(() => true, () => false)
  }
}
