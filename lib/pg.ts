...
export class PgClient {
  ...
  getChatById = (id: number): Promise<DBChat | undefined> =>
    this.pg.query("select * from chats where chat_id = ? limit 1", [id]).then(
      (r) => r.rows[0],
      () => undefined,
    );
  ...
  getChat = (chat: Chat): Promise<DBChat | undefined> =>
    this.pg
      .query("select * from chats where chat_id = ? limit 1", [chat.id])
      .then(
        (r) => r.rows[0],
        () => undefined,
      );
  ...
  getChats = (): Promise<DBChat[]> =>
    this.pg.query("select * from chats").then((r) => r.rows);
  ...
  addChat = async (
    ...
    `;
    return this.pg.query(query, [chat.id, chat.username, chat.title]).then(
      (r) => r.rowCount > 1,
      () => false,
    );
  };

  setChatProp = <T>(chat: Chat, prop: string, value: T) =>
    this.pg.query("update chats set ? = ? where chat_id = ?", [
      prop,
      value,
      chat.id,
    ]);
  ...
  updateChatProp = async <K extends keyof Chat>(
    ...
    const row = await this.pg
      .query("select ? from chats where chat_id = ? limit 1", [
        propName,
        chat.id,
      ])
      .then((r) => r.rows[0]);
    ...
    return this.pg
      .query("update chats set ? = ? where chat_id = ?", [
        propName,
        nextVal,
        chat.id,
      ])
